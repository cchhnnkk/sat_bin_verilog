...
def gen_regs_vid_state():
	...
	# for i in xrange(24):
	# 	print "need_update[%d] <= need_shift[%d]? need_update[%d]:need_update[%d];"%(i,i,i+1,i)
	for i in xrange(24):
...
# gen_clause_array()
# gen_base()
#for i in xrange(12): print 'wire [WIDTH_VAR_STATES-1 : 0] max_lvl_%d = var_level_o_%d>var_level_o_%d? var_level_o_%d:var_level_o_%d;'%(i+1, 2*i+1, 2*i, 2*i+1, 2*i)
